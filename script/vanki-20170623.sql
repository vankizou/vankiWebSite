...
CREATE TABLE `t_note` (
  ...
  `STATUS` int(2) DEFAULT '1' COMMENT '-1待审核，0不通过，1通过，2全部通过（比如：主页）',
  `STATUS_DESCRIPTION` varchar(200) DEFAULT NULL COMMENT '审核理由',
  `IS_DEL` int(2) DEFAULT '0' COMMENT '是否被删除。0否，1是，默认0',
  PRIMARY KEY (`ID`),
  KEY `USER_ID` (`USER_ID`),
  KEY `PARENT_ID` (`PARENT_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=129 DEFAULT CHARSET=utf8 COMMENT='笔记';
...
CREATE TABLE `t_note_detail` (
  ...
  PRIMARY KEY (`ID`),
  KEY `NOTE_ID` (`NOTE_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=96 DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='笔记详情';
...
) ENGINE=InnoDB AUTO_INCREMENT=1023 DEFAULT CHARSET=utf8mb4 ROW_FORMAT=DYNAMIC COMMENT='用户信息';
